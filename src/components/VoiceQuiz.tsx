...
import type { QuizQuestion } from "./QuizTaker";
...
function speakableQuestion(q: QuizQuestion): string {
  if (q.type === "open") return q.question;
  const opts = q.options
    .map((opt, i) => `Option ${NUMBER_WORDS[i] ?? i + 1}: ${opt}.`)
    .join(" ");
...
export function VoiceQuiz({
    ...
    (async () => {
      try {
        ...
        setPhase({
          kind: "error",
          message:
            "Microphone access is required for voice mode. Allow mic access and reload.",
        });
      }
    })();
    return () => {
  ...
  function runSpeaking(qIdx: number) {
    ...
    const spoken = speakableQuestion(q);
    const text =
      fails === 0
        ? ack
          ? `${ack} ${spoken}`
          : spoken
        : fails === 1
        ? `Sorry, I didn't catch that. ${spoken}`
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#0b0b14] text-white">
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-6 gap-10">
        ...
        <div className="min-h-[140px] max-w-xl w-full text-center">
          {currentQ && phase.kind === "speaking" && (
            <div key={currentQ.id} className="space-y-4 animate-fadeIn">
